...
- **Integration maturity:** Server catalog + cart/orders/wishlist/reviews API (referential delete guards, rename propagation, attribute usage counts); admin catalog CRUD + product attribute picker + review moderation + feature-filtered nav; client `/shop`/`/product/[slug]` + customer auth + wishlist sync when `features.wishlist` + reviews when `features.reviews`.
