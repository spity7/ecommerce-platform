...
import { babyFoodProducts } from "@/data/products/foods";
...
export default function Products1() {
  ...
  const filteredProducts = useMemo(() => {
    if (activeTab === "view-all") return babyFoodProducts;

    return babyFoodProducts.filter((product) =>
      product.demoTab?.includes(activeTab)
    );
  }, [activeTab]);

  return (
    <div className="rbt-component-area rbt-products-area rbt-bg-color-gray-light rbt-section-gap rbt-section-has-edge-divider">
      <div className="rbt-edge-top rbt-text-color-gray-light">
        <WaveDividerIcon />
      </div>
      <div className="rbt-edge-bottom rbt-text-color-gray-light">
        <WaveBorderIcon />
      </div>
      <div className="rbt-wider-container bg-transparent p--0">
        <div>
          <div className="row">
          ...
          <div className="row row--12 mt_dec--24">
            ...
            {filteredProducts.map((product, i) => (
              ...
              >
                <ProductCard8 product={product} animationOrder={i + 1} />
              </div>
            ))}
            {/* End Single Card  */}
          </div>
