import { campingCategories } from "@/data/categories";

import Image from "next/image";
import Link from "next/link";

const CAMPING_CATEGORY_ANIMATION_CLASSES = [
  "animation-order-1",
  "animation-order-2",
  "animation-order-3",
  "animation-order-4",
    ...
    <div className="rbt-component-area rbt-categories-area rbt-bg-color-white rbt-section-gap">
      <div className="container">
          ...
          <div className="col-lg-12 d-flex justify-content-between flex-row align-items-end mb--40 flex-wrap rbt-gap--16">
            <div className="rbt-component-section-title rbt-gap--4 mb--0 p-0 border-0">
              <h2 className="rbt-title rbt-scroll-trigger fade_in animation-order-1">
                Discover the
                <span className="rbt-bold--text ml--4">World Of Outdoors</span>
              </h2>
            </div>
            <Link
        ...
        <div className="row row--12 mt_dec--24">
          {campingCategories.map((item, index) => {
            const animationClass =
              CAMPING_CATEGORY_ANIMATION_CLASSES[index] ?? "animation-order-1";
