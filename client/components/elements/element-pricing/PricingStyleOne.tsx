...
import { pricingStyleOnePlans } from "@/data/pricing";
import { useState } from "react";

function PricingStyleOne() {
  const [isYearlyPlan, setIsYearlyPlan] = useState(false);

  return (
    <div
      id="rbt-pricing-block-01"
      className="rbt-pricing-area rbt-bg-color-white rbt-section-gap"
    >
                ...
                <div className="pricing-body">
                  <ul className="list-item">
                    {plan.features.map((feature) => (
                      <li key={`${plan.title}-${feature.text}`}>
                        <i className="fa-regular fa-check" /> {feature.text}
                      </li>
                    ))}
                  </ul>
                </div>
