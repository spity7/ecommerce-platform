...
export default function HeroModal({ id, title, count }: HeroModalProps) {
                ...
                <div className="container">
                  <div className="row row--12 mt_dec--24">
                    <div className="col-12 col-md-6 col-lg-4 mt--24">
                      {blocsHeroCol1.map((entry, index) => (
                        <BlocsEntry key={index} entry={entry} />
                      ))}
                    </div>
                    <div className="col-12 col-md-6 col-lg-4 mt--24">
                      {blocsHeroCol2.map((entry, index) => (
                        <BlocsEntry key={index} entry={entry} />
                      ))}
                    </div>
                    <div className="col-12 col-md-6 col-lg-4 mt--24">
                      {blocsHeroCol3.map((entry, index) => (
                        <BlocsEntry key={index} entry={entry} />
                      ))}
                    </div>
                  </div>
