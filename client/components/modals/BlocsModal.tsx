...
import {
  blocsContentCol1, blocsContentCol2, blocsContentCol3,
} from "@/data/splash";

interface BlocsModalProps {
  id: string;
  title: string;
  count: string;
}
...
export default function BlocsModal({ id, title, count }: BlocsModalProps) {
        ...
        <div className="modal-content">
          ...
          <div className="rbt-top-folder-shape-wrapper">
            <div className="rbt-bg-color-white rbt-content-trs-portion rbt-element-block-presentation">
              <h4 className="rbt-title mb--0 text-center">
                {title}
                ...
                <div className="container">
                  <div className="row row--12 mt_dec--24">
                    <div className="col-12 col-md-6 col-lg-4 mt--24">
                      {blocsContentCol1.map((entry, index) => (
                        <BlocsEntry key={index} entry={entry} />
                      ))}
                    </div>
                    <div className="col-12 col-md-6 col-lg-4 mt--24">
                      {blocsContentCol2.map((entry, index) => (
                        <BlocsEntry key={index} entry={entry} />
                      ))}
                    </div>
                    <div className="col-12 col-md-6 col-lg-4 mt--24">
                      {blocsContentCol3.map((entry, index) => (
                        <BlocsEntry key={index} entry={entry} />
                      ))}
                    </div>
                  </div>
