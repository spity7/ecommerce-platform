...
**Last reviewed:** 2026-09-21. See [AI-INDEX.md](AI-INDEX.md) for the full doc map and tree.
...
| Admin  | Catalog CRUD + sign-in + orders list/detail + **review moderation**; rich catalog forms; product attribute picker; nav filtered by `SiteConfig.features`; dashboard greeting uses signed-in name; dashboard category strip from catalog API                                                                                                         | Other dashboard widgets, customers, coupons, settings, reports (~30 demo pages) |
...
- `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS` — server only, per **deployment** (one site per API instance). Transactional and storefront display From/mailto both use `SiteConfig.contact.email` (no separate `EMAIL_FROM` env).
