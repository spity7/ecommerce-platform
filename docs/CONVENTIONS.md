...
| Product card badges | Resolved on the API as `ProductDto.badges` (max 2, theme classes `rbt-product-badge-*`). Auto rules (sale, new, stock, ratings, units sold) + admin `merchandising.manualBadges` (fixed `ProductBadgeKind` registry only; optional `label` override per kind). Storefront: `ProductCardImageBadges`; map via `mapProductDtoToStorefront`. Registry + resolver in `@platform/shared`. Site thresholds: `SiteConfig.merchandising`.                                                                        |
