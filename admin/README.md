...
**Template only:** dashboard home widgets (greeting uses the signed-in admin name; the category strip uses live catalog categories with a template fallback), coupons, settings, reports, and most other nav items use demo data. The legacy `products/demo/edit` form (comma-separated “Product Badges”) is UI preview only — use **Products → New/Edit** (`ProductCatalogForm`) for API-backed storefront badges. See [docs/ROUTES.md](../docs/ROUTES.md).
